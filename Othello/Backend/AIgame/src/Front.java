...
public class Front {
	...
	void getUserInput(int turn)
	{
		...
		insertPosition(x,y,turn,gameBoard);
		
		
		//in.close();
	}
	
	void insertPosition(int x , int y , int turn ,int [][] gameBoard)
	{
		if(turn == 1)
		{
		...
		else if(choice == 2)
		{
			...
			while(true)
			{
				...
				if(s1.countSuggestion(f1.gameBoard) == 0)
				{
					...
					if(turn == 0)
					{
					 int val =  m1.minimax(f1.gameBoard,0,true,9 , Integer.MIN_VALUE , Integer.MAX_VALUE);
					}
					if(turn ==1)
					f1.getUserInput(turn);
					t = -t;
