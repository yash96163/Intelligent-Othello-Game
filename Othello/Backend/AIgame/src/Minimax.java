...
public class Minimax {
	...
	int minimax(int gameBoard[][] , int depth , boolean isMax , int h , int alpha , int beta)
	{
		if(sobj.countSuggestion(gameBoard) == 1)
			isMax = !isMax;
			...
			for(int i = 0 ; i < 8 ; i++)
			{
				...
				for(int j = 0 ; j < 8 ; j++)
				{
					if(gameBoard[i][j] == 2)
					{
						...
						int value = minimax(arr , depth+1 , false , h , alpha , beta);
						if(value > max)
						{
							max = value;
							posi = i;
					...
					else
					{
						...
						for(int i = 0 ; i < 8 ; i++)
						{
							for(int j = 0 ; j < 8 ; j++)
							{
								if(gameBoard[i][j] == 2)
								{
									f1.insertPosition(i, j, 0, gameBoard);
									flag = 1;
									break;
								}
							}
			...
			for(int i = 0 ; i < 8 ; i++)
			{
				...
				for(int j = 0 ; j < 8 ; j++)
				{
					if(gameBoard[i][j] == 2)
					{
						int arr[][] = new int[8][8];
						arr = copyArray(gameBoard);
						f1.insertPosition(i, j,1, arr);
						int value = minimax(arr , depth+1 , true , h , alpha , beta);
						if(value < min)
						{
							min = value;
						//	posi = i;
